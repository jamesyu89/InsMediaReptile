            ...
            while (true)
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    try
                    {
                        ...
                        if (i == 4)
                        {
                            Console.WriteLine("当前程序已重试4次，未正常获取到数据，程序正在退出...");
                            Environment.Exit(-1);//退出
                        }
                    }
                    catch (Exception e)
                    {
                        Console.WriteLine($"查询下载任务出现异常，异常信息:{e.Message}，正在进行第{i + 1}次重试...");
                        queryFlag -= 1;
                    }
                }
                ...
                //生成下载任务，并保存到指定目录
                Parallel.For(0, downloadTasks.Count, (i) =>
                {
                    System.Threading.Tasks.Task.Run(() =>
                    {
                        try
                        {
                            Console.WriteLine("[正在下载资源：" + downloadTasks[i].HttpUrl + "]");
                            // 设置参数
