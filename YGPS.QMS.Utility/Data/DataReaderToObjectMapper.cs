...
{
    internal class DataReaderToObjectMapper<T> : ObjectsMapper<DbDataReader, T>
    {
        ...
        private class DbReaderMappingConfig : IMappingConfigurator
        {
            ...
            private class ReaderValuesExtrator<T1>
            {
                public readonly string fieldName;
                public readonly Func<int, DbDataReader, T1> valueExtractor;
                public int fieldNum;

                public ReaderValuesExtrator(string fieldName, Func<int, DbDataReader, T1> valueExtractor)
                {
                    fieldNum = -1;
                ...
                public Delegate ExtrationDelegate
                {
                    get
                    {
                        return (ValueGetter<T1>)
                            (
                                (value, state) =>
                                {
                                    return ValueToWrite<T1>.ReturnValue(GetValue((DbDataReader) state));
                                }
                                );
                    }
                }

                private T1 GetValue(DbDataReader reader)
                {
                    if (fieldNum == -1)
                    {
                        fieldNum = reader.GetOrdinal(fieldName);
                    }
                    return reader.IsDBNull(fieldNum) ? default(T1) : valueExtractor(fieldNum, reader);
                }
            }
        }
    }
